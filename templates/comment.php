...
echo '<div class="media-body">';
	...
	echo '<time datetime="' . comment_date( 'c' ) . '"><a href="' . htmlspecialchars( get_comment_link( $comment->comment_ID ) ) . '">';
		printf( __( '%1$s', 'stachestack' ), get_comment_date(),  get_comment_time() );
	echo '</a></time>';

	edit_comment_link( __( '(Edit)', 'stachestack' ), '', '' );

	if ( $comment->comment_approved == '0' ) {
		echo $ss_framework->alert( 'info', __( 'Your comment is awaiting moderation.', 'stachestack' ) );
	}
