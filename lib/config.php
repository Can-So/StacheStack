 ...
 */
function stachestack_display_primary_sidebar() {
	$sidebar_config = new StacheStack_Sidebar(
		array(
			'is_404',
			'is_front_page'
		),
	...
	return apply_filters( 'stachestack_display_primary_sidebar', $sidebar_config->display );
}
 ...
 * See lib/sidebar.php for more details
 */
function stachestack_display_secondary_sidebar() {
	$sidebar_config = new StacheStack_Sidebar(
		array(
			'is_404',
			'is_front_page'
	...
	return apply_filters( 'stachestack_display_secondary_sidebar', $sidebar_config->display );
}


if ( ! function_exists( 'stachestack_section_class' ) ) :
/*
 ...
 */
function stachestack_mp_wrap_div_open() {
	global $ss_framework;

	if ( $ss_framework->include_wrapper() ) {
		echo '<div class="mp_wrap ' . stachestack_section_class( 'wrapper' ) . '">' . $ss_framework->open_row( 'div' );
	}
}

/*
 * Closes the <div> opened by the stachestack_mp_wrap_div_open() function.
 */
function stachestack_mp_wrap_div_close() {
	global $ss_framework;
 ...
 *
 * Uses the stachestack_mp_wrap_div_open() and stachestack_mp_wrap_div_close() functions.
 */
...
add_action( 'wp', 'stachestack_mp_wrap_div_toggler' );
