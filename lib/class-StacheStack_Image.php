...
if ( ! class_exists( 'StacheStack_Image' ) ) {

	/**
	* The Image handling class
	*/
	class StacheStack_Image {

		function __construct() {
		}

		/*
		 * Proxy function to be called whenever an image is used. If you wish to resize, use stachestack_image_resize()
		 */
		public static function image( $img ) {
			...
			return stachestack_image_resize( $img );
		}
		...
		public static function _resize( $url, $width = NULL, $height = NULL, $crop = true, $retina = false ) {
			...
			if ( empty( $url ) ) {
				return new WP_Error( 'no_image_url', __( 'No image URL has been entered.', 'stachestack' ), $url );
			}
