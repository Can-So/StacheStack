...
if ( ! function_exists( 'stachestack_getVariable' ) ) :
/*
 * Gets the current values from REDUX, and if not there, grabs the defaults
 */
function stachestack_getVariable( $name, $key = false ) {
	global $redux;
	$options = $redux;
...
if ( ! function_exists( 'stachestack_password_form' ) ) :
/*
 * Replace the password forms with a bootstrap-formatted version.
 */
function stachestack_password_form() {
	global $post, $ss_framework;
	$label    = 'pwbox-' . ( empty( $post->ID ) ? rand() : $post->ID );
	$content  = '<form action="';
	$content .= esc_url( site_url( 'wp-login.php?action=postpass', 'login_post' ) );
	$content .= '" method="post">';
	$content .= __( 'This post is password protected. To view it please enter your password below:', 'stachestack' );
...
add_filter( 'the_password_form', 'stachestack_password_form' );


if ( ! function_exists( 'stachestack_replace_reply_link_class' ) ) :
/*
 * Apply the proper classes to comment reply links
 */
function stachestack_replace_reply_link_class( $class ) {
	global $ss_framework;
	$class = str_replace( "class='comment-reply-link", "class='comment-reply-link " . $ss_framework->button_classes( 'success', 'small' ), $class );
	return $class;
}
endif;
add_filter('comment_reply_link', 'stachestack_replace_reply_link_class');


if ( ! function_exists( 'stachestack_init_filesystem' ) ) :
/*
 * Initialize the Wordpress filesystem, no more using file_put_contents function
 */
function stachestack_init_filesystem() {
...
add_filter('init', 'stachestack_init_filesystem');


if ( ! function_exists( 'stachestack_array_delete' ) ) :
/*
 * Unset a row from an array.
 */
function stachestack_array_delete( $idx, $array ) {  
	unset( $array[$idx] );
 ...
 * Canonical URLs
 */
function stachestack_rel_canonical() {
	global $wp_the_query;
...
add_action( 'init', 'stachestack_head_cleanup' );
 ...
 */
function stachestack_remove_dashboard_widgets() {
	remove_meta_box( 'dashboard_incoming_links', 'dashboard', 'normal' );
	remove_meta_box( 'dashboard_plugins', 'dashboard', 'normal' );
	remove_meta_box( 'dashboard_primary', 'dashboard', 'normal' );
	remove_meta_box( 'dashboard_secondary', 'dashboard', 'normal' );
}
add_action( 'admin_init', 'stachestack_remove_dashboard_widgets' );


function stachestack_process_font( $font ) {
