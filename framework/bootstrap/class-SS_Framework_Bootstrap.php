	...
	class SS_Framework_Bootstrap extends SS_Framework_Core {
		...
		public function __construct() {
			...
			if ( class_exists( 'ReduxFrameworkPlugin' ) ) {
				...
				include_once( SS_FRAMEWORK_PATH . '/includes/class-StacheStack_Options.php' );         // Redux Options
				include_once( SS_FRAMEWORK_PATH . '/includes/class-StacheStack_Advanced.php' );        // Advanced
				include_once( SS_FRAMEWORK_PATH . '/includes/class-StacheStack_Background.php' );      // Background
				include_once( SS_FRAMEWORK_PATH . '/includes/class-StacheStack_Branding.php' );        // Branding
				include_once( SS_FRAMEWORK_PATH . '/includes/class-StacheStack_Blog.php' );            // Blog
				include_once( SS_FRAMEWORK_PATH . '/includes/class-StacheStack_Breadcrumbs.php' );     // Breadcrumbs
				include_once( SS_FRAMEWORK_PATH . '/includes/class-StacheStack_Header.php' );          // Header
				include_once( SS_FRAMEWORK_PATH . '/includes/class-StacheStack_Typography.php' );      // Typography
				include_once( SS_FRAMEWORK_PATH . '/includes/class-StacheStack_Footer.php' );          // Footer
				include_once( SS_FRAMEWORK_PATH . '/includes/class-StacheStack_Social.php' );          // Social
				include_once( SS_FRAMEWORK_PATH . '/includes/class-StacheStack_Layout.php' );          // layout
				include_once( SS_FRAMEWORK_PATH . '/includes/class-StacheStack_Jumbotron.php' );       // Jumbotron
				include_once( SS_FRAMEWORK_PATH . '/includes/class-StacheStack_Menus.php' );           // Menus
				include_once( SS_FRAMEWORK_PATH . '/includes/class-StacheStack_Nav_Walker.php' );      // NavWalker
				include_once( SS_FRAMEWORK_PATH . '/includes/class-StacheStack_Nav_Menu_Widget.php' ); // NavMenus
				include_once( SS_FRAMEWORK_PATH . '/includes/class-StacheStack_Navlist_Walker.php' );  // NavLists
				...
				global $ss_layout;
				$ss_layout      = new StacheStack_Layout();

				global $ss_background;
				$ss_background  = new StacheStack_Background();

				global $ss_advanced;
				$ss_advanced    = new StacheStack_Advanced();

				global $ss_branding;
				$ss_branding    = new StacheStack_Branding();

				global $ss_blog;
				$ss_blog        = new StacheStack_Blog();

				global $ss_footer;
				$ss_footer      = new StacheStack_Footer();

				global $ss_headers;
				$ss_headers     = new StacheStack_Header();

				global $ss_jumbotron;
				$ss_jumbotron   = new StacheStack_Jumbotron();

				global $ss_menus;
				$ss_menus       = new StacheStack_Menus();

				global $ss_typography;
				$ss_typography  = new StacheStack_Typography();

				global $ss_breadcrumbs;
				$ss_breadcrumbs = new StacheStack_Breadcrumbs();

				global $ss_social;
				$ss_social      = new StacheStack_Social();

				add_filter( 'stachestack_compiler', array( $this, 'styles_filter' ) );

				if ( isset( $ss_settings['navbar_social'] ) && $ss_settings['navbar_social'] == 1 ) {
					if ( $ss_settings['navbar_social_style'] == 1 ) {
						add_action( 'stachestack_inside_nav_end', array( $this, 'navbar_social_bar' ) );
					} else {
						add_action( 'stachestack_inside_nav_end', array( $this, 'navbar_social_links' ) );
					}
				}
			...
			add_action( 'stachestack_pre_wrap', array( $this, 'breadcrumbs' ), 99 );
		...
		function navlist_widget_init() {
			...
			register_widget( 'StacheStack_Nav_Menu_Widget' );
		}
		...
		function nav_menu_css_class( $classes, $item ) {
			...
			$classes[] = 'menu-' . stachestack_transliterate( $slug );
		 ...
		 * Use StacheStack_Nav_Walker() by default
		 */
		function nav_menu_args( $args = '' ) {
			...
			if ( ! $args['walker'] ) {
				$nav_menu_args['walker'] = new StacheStack_Nav_Walker();
			}

			if ( ! $args['fallback_cb'] ) {
				$nav_menu_args['fallback_cb'] = 'StacheStack_Nav_Walker::fallback';
			}
		...
		function breadcrumbs() {
			...
			if ( class_exists( 'StacheStack_Breadcrumbs' ) ) {
		...
		public function styles() {
			...
			$body_bg = '#' . str_replace( '#', '', StacheStack_Color::sanitize_hex( $bg ) );

			// Calculate the gray shadows based on the body background.
			// We basically create 2 "presets": light and dark.
			if ( StacheStack_Color::get_brightness( $body_bg ) > 80 ) {
			...
			$bg_brightness = StacheStack_Color::get_brightness( $body_bg );
			 ...
			 * BRANDING
			 */
			$brand_primary = '#' . str_replace( '#', '', StacheStack_Color::sanitize_hex( $ss_settings['color_brand_primary'] ) );
			$brand_success = '#' . str_replace( '#', '', StacheStack_Color::sanitize_hex( $ss_settings['color_brand_success'] ) );
			$brand_warning = '#' . str_replace( '#', '', StacheStack_Color::sanitize_hex( $ss_settings['color_brand_warning'] ) );
			$brand_danger  = '#' . str_replace( '#', '', StacheStack_Color::sanitize_hex( $ss_settings['color_brand_danger'] ) );
			$brand_info    = '#' . str_replace( '#', '', StacheStack_Color::sanitize_hex( $ss_settings['color_brand_info'] ) );

			$link_hover_color = ( StacheStack_Color::get_brightness( $brand_primary ) > 50 ) ? 'darken(@link-color, 15%)' : 'lighten(@link-color, 15%)';

			$brand_primary_brightness = StacheStack_Color::get_brightness( $brand_primary );
			$brand_success_brightness = StacheStack_Color::get_brightness( $brand_success );
			$brand_warning_brightness = StacheStack_Color::get_brightness( $brand_warning );
			$brand_danger_brightness  = StacheStack_Color::get_brightness( $brand_danger );
			$brand_info_brightness    = StacheStack_Color::get_brightness( $brand_info );
			...
			$input_border_focus = ( StacheStack_Color::get_brightness( $brand_primary ) < 195 ) ? 'lighten(@brand-primary, 10%)' : 'darken(@brand-primary, 10%)';
			$navbar_border      = ( StacheStack_Color::get_brightness( $brand_primary ) < 50 ) ? 'lighten(@navbar-default-bg, 6.5%)' : 'darken(@navbar-default-bg, 6.5%)';
			 ...
			 * JUMBOTRON
			 */
			$font_jumbotron         = stachestack_process_font( $ss_settings['font_jumbotron'] );
			$jumbotron_bg           = $ss_settings['jumbo_bg'];
			$jumbotron_bg           = '#' . str_replace( '#', '', StacheStack_Color::sanitize_hex( $jumbotron_bg['background-color'] ) );
			$jumbotron_text_color   = '#' . str_replace( '#', '', $font_jumbotron['color'] );

			if ( $ss_settings['font_jumbotron_heading_custom'] == 1 ) {
				$font_jumbotron_headers = stachestack_process_font( $ss_settings['font_jumbotron_headers'] );
				...
				$jumbotron_headers_text_color  = '#' . str_replace( '#', '', StacheStack_Color::sanitize_hex( $font_jumbotron_headers['color'] ) );
			...
			// StacheStack-specific variables
			// --------------------------------------------------
			 ...
			 * MENUS
			 */
			$font_brand        = stachestack_process_font( $ss_settings['font_brand'] );

			$font_navbar       = stachestack_process_font( $ss_settings['font_navbar'] );
			$navbar_bg         = '#' . str_replace( '#', '', StacheStack_Color::sanitize_hex( $ss_settings['navbar_bg'] ) );
			...
			$navbar_border     = ( StacheStack_Color::get_brightness( $navbar_bg ) < 50 ) ? 'lighten(@navbar-default-bg, 6.5%)' : 'darken(@navbar-default-bg, 6.5%)';
			$gfb = $ss_settings['grid_float_breakpoint'];

			if ( StacheStack_Color::get_brightness( $navbar_bg ) < 165 ) {
			...
			try {
				...
				$parser = new Less_Parser( $options );
                                $parser->ModifyVars(array(
                                   'assets-url' =>  '"' . STACHESTACK_ASSETS_URL . '"'
                                ));
