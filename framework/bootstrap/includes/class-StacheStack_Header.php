...
if ( ! class_exists( 'StacheStack_Header' ) ) {

	/**
	* The Header module
	*/
	class StacheStack_Header {

		function __construct() {
			add_action( 'widgets_init',       array( $this, 'header_widgets_init' ), 30 );
			add_action( 'stachestack_pre_wrap', array( $this, 'branding' ), 3 );
		...
		function header_widgets_init() {
			register_sidebar( array(
				'name'          => __( 'Header Area', 'stachestack' ),
				'id'            => 'header-area',
				'before_widget' => '<div>',
				'after_widget'  => '</div>',
				'before_title'  => '<h1>',
		...
		function branding() {
			...
			if ( $ss_settings['header_toggle'] == 1 ) {
				...
				if ( $ss_settings['header_branding'] == 1 ) {
					echo '<a class="brand-logo" href="' . home_url() . '/"><h1>' . StacheStack_Branding::logo() . '</h1></a>';
				}
