...
if ( !class_exists( 'StacheStack_Advanced' ) ) {

	/**
	* The "Advanced" module
	*/
	class StacheStack_Advanced {
		...
		*/
		...
		function user_css() {
			...
			if ( trim( $header_scripts ) != '' ) {
				wp_add_inline_style( 'stachestack_css', $header_scripts );
			}
		}
		...
		function scripts() {
			...
			if ( $settings['retina_toggle'] == 1 ) {
				wp_register_script( 'retinajs', STACHESTACK_ASSETS_URL . '/js/vendor/retina.js', false, null, true );
