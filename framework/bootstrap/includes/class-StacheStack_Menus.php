...
if ( ! class_exists( 'StacheStack_Menus' ) ) {
	...
	* The "Menus" module
	*/
	class StacheStack_Menus {

		function __construct() {
			global $ss_settings;

			add_filter( 'stachestack_nav_class',        array( $this, 'nav_class' ) );
			add_action( 'stachestack_inside_nav_begin', array( $this, 'navbar_pre_searchbox' ), 11 );
			add_filter( 'stachestack_navbar_class',     array( $this, 'navbar_class' ) );
			add_action( 'wp_enqueue_scripts',         array( $this, 'navbar_css' ), 101 );
			add_filter( 'stachestack_navbar_brand',     array( $this, 'navbar_brand' ) );
			add_filter( 'body_class',                 array( $this, 'navbar_body_class' ) );
			add_action( 'widgets_init',               array( $this, 'sl_widgets_init' ), 40 );
			add_action( 'stachestack_post_main_nav',    array( $this, 'navbar_sidebar' ) );
			add_action( 'stachestack_pre_wrap',         array( $this, 'secondary_navbar' ) );
			add_action( 'widgets_init',               array( $this, 'slidedown_widgets_init' ), 50 );
			add_action( 'wp_enqueue_scripts',         array( $this, 'megadrop_script' ), 200 );
			add_action( 'stachestack_pre_wrap',         array( $this, 'content_wrapper_static_left_open' ) );
			add_action( 'stachestack_after_footer',     array( $this, 'content_wrapper_static_left_close' ), 1 );
			...
			if ( isset( $ss_settings['navbar_toggle'] ) ) {
				...
				if ( $ss_settings['navbar_toggle'] == 'left' ) {
					$hook_navbar_slidedown_toggle = 'stachestack_pre_content';
				} else {
					$hook_navbar_slidedown_toggle = 'stachestack_inside_nav_begin';
				}
			...
			if ( isset( $ss_settings['navbar_toggle'] ) ) {
				...
				if ( $ss_settings['navbar_toggle'] == 'left' ) {
					$hook_navbar_slidedown_content = 'stachestack_pre_content';
				} else {
					$hook_navbar_slidedown_content = 'stachestack_do_navbar';
				}
		...
		function navbar_pre_searchbox() {
			...
			if ( $show_searchbox == '1' ) : ?>
				<form role="search" method="get" id="searchform" class="form-search pull-right navbar-form" action="<?php echo home_url('/'); ?>">
					<label class="hide" for="s"><?php _e('Search for:', 'stachestack'); ?></label>
					<input type="text" value="<?php if (is_search()) { echo get_search_query(); } ?>" name="s" id="s" class="form-control search-query" placeholder="<?php _e('Search', 'stachestack'); ?> <?php bloginfo('name'); ?>">
				</form>
			<?php endif;
		}
		...
		function sl_widgets_init() {
			register_sidebar( array(
				'name'          => __( 'In-Navbar Widget Area', 'stachestack' ),
				'id'            => 'navbar',
				'description'   => __( 'This widget area will show up in your NavBars. This is most useful when using a static-left navbar.', 'stachestack' ),
				...
				<?php echo $ss_framework->open_container( 'div' ); ?>
					<header class="secondary navbar navbar-default <?php echo self::navbar_class( 'secondary' ); ?>" role="banner">
						...
						<nav class="nav-secondary navbar-collapse collapse" role="navigation">
							<?php wp_nav_menu( array( 'theme_location' => 'secondary_navigation', 'menu_class' => apply_filters( 'stachestack_nav_class', 'navbar-nav nav' ) ) ); ?>
						</nav>
					</header>
		...
		function slidedown_widgets_init() {
			...
			register_sidebar( array(
				'name'          => __( 'Navbar Slide-Down Top', 'stachestack' ),
			...
			register_sidebar( array(
				'name'          => __( 'Navbar Slide-Down 1', 'stachestack' ),
			...
			register_sidebar( array(
				'name'          => __( 'Navbar Slide-Down 2', 'stachestack' ),
			...
			register_sidebar( array(
				'name'          => __( 'Navbar Slide-Down 3', 'stachestack' ),
			...
			register_sidebar( array(
				'name'          => __( 'Navbar Slide-Down 4', 'stachestack' ),
		...
		function megadrop_script() {
			if ( is_active_sidebar( 'navbar-slide-down-top' ) || is_active_sidebar( 'navbar-slide-down-1' ) || is_active_sidebar( 'navbar-slide-down-2' ) || is_active_sidebar( 'navbar-slide-down-3' ) || is_active_sidebar( 'navbar-slide-down-4' ) ) {
				wp_register_script( 'stachestack_megadrop', get_template_directory_uri() . '/assets/js/megadrop.js', false, null, false );
				wp_enqueue_script( 'stachestack_megadrop' );
			}
		}
	}
}
