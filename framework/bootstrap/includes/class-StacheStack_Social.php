...
if ( ! class_exists( 'StacheStack_Social' ) ) {
	/**
	* The "Social" modue
	*/
	class StacheStack_Social {

		function __construct() {
			global $ss_settings;

			// When on a BuddyPress, disable social shares.
			if ( class_exists( 'BuddyPress' ) && stachestack_is_bp() ) {
			...
			if ( isset( $ss_settings['social_sharing_archives'] ) && $ss_settings['social_sharing_archives'] == 1 ) {
				add_action( 'stachestack_entry_footer', array( $this, 'social_sharing' ), 5 );
			}
			...
			if ( isset( $ss_settings['social_sharing_single_post'] ) && $ss_settings['social_sharing_single_post'] == 1 ) {
				if ( $ss_settings['social_sharing_location'] == 'top' ) {
					add_action( 'stachestack_single_pre_content',   array( $this, 'social_sharing' ), 5 );
				} elseif ( $ss_settings['social_sharing_location'] == 'bottom' ) {
					add_action( 'stachestack_single_after_content', array( $this, 'social_sharing' ), 5 );
			...
			if ( isset( $ss_settings['social_sharing_single_page'] ) && $ss_settings['social_sharing_single_page'] == 1 ) {
				if ( $ss_settings['social_sharing_location'] == 'top' ) {
					add_action( 'stachestack_page_pre_content',   array( $this, 'social_sharing' ), 5 );
				} elseif ( $ss_settings['social_sharing_location'] == 'bottom' ) {
					add_action( 'stachestack_page_after_content', array( $this, 'social_sharing' ), 5 );
				} elseif ( $ss_settings['social_sharing_location'] == 'both' ) {
					add_action( 'stachestack_page_pre_content',   array( $this, 'social_sharing' ), 5 );
					add_action( 'stachestack_page_after_content', array( $this, 'social_sharing' ), 5 );
				}
