...
if ( ! class_exists( 'StacheStack_Layout' ) ) {

	/**
	* The "Layout Module"
	*/
	class StacheStack_Layout {

		function __construct() {
			global $ss_settings;

			add_filter( 'stachestack_section_class_wrapper',   array( $this, 'apply_layout_classes_wrapper'   )     );
			add_filter( 'stachestack_section_class_main',      array( $this, 'apply_layout_classes_main'      )     );
			add_filter( 'stachestack_section_class_primary',   array( $this, 'apply_layout_classes_primary'   )     );
			add_filter( 'stachestack_section_class_secondary', array( $this, 'apply_layout_classes_secondary' )     );
			add_filter( 'stachestack_container_class',         array( $this, 'container_class'                )     );
			add_filter( 'body_class',                        array( $this, 'layout_body_class'              )     );
			add_filter( 'stachestack_navbar_container_class',  array( $this, 'navbar_container_class'         )     );
			...
			add_action( 'stachestack_pre_footer',   array( $this, 'boxed_container_div_open' ), 1 );
			add_action( 'stachestack_do_navbar',    array( $this, 'boxed_container_div_close' ), 99 );
			add_action( 'stachestack_after_footer', array( $this, 'boxed_container_div_close' ), 899 );
			...
			if ( isset( $ss_settings['widgets_mode'] ) ) {
				...
				if ( $widgets_mode == 0 || $widgets_mode == 1 ) {
					add_filter( 'stachestack_widgets_class',        array( $this, 'alter_widgets_class'        ) );
					add_filter( 'stachestack_widgets_before_title', array( $this, 'alter_widgets_before_title' ) );
					add_filter( 'stachestack_widgets_after_title',  array( $this, 'alter_widgets_after_title'  ) );
				}
			}
				...
				if ( $ss_settings['cpt_layout_toggle'] == 1 ) {
					if ( ! is_page_template() ) {
						...
						foreach ( $post_types as $post_type ) {
							$stachestack_layout = ( is_singular( $post_type ) ) ? intval( $ss_settings[$post_type . '_layout'] ) : $stachestack_layout;
						}
					}
				}

				if ( ! is_active_sidebar( 'sidebar-secondary' ) && is_active_sidebar( 'sidebar-primary' ) && $stachestack_layout == 5 ) {
					$stachestack_layout = 3;
				}
			}
			return $stachestack_layout;
		}
		...
		function set_layout( $val ) {
			global $stachestack_layout, $redux;
			$stachestack_layout = intval( $val );
		}
		...
		public static function section_class_ext( $target, $echo = false ) {
			...
			if ( stachestack_display_primary_sidebar() && stachestack_display_secondary_sidebar() ) {
			...
			} elseif ( stachestack_display_primary_sidebar() && ! stachestack_display_secondary_sidebar() ) {
		...
		public static function container_class() {
			...
			if ( class_exists( 'StacheStack_Menus' ) ) {
		...
		function navbar_container_class() {
			...
			// override if navbar module exists and 'navbar-toggle' is set to left.
			if ( class_exists( 'StacheStackMenus' ) ) {
				if ( $ss_settings['navbar_toggle'] == 'left' ) {
					$class = 'fluid';
				}
			}
		...
		function control_primary_sidebar_display() {
			...
			if ( self::get_layout() == 0 ) {
				add_filter( 'stachestack_display_primary_sidebar', '__return_false' );
			}

			if ( is_front_page() && $layout_sidebar_on_front == 1 && self::get_layout() != 0 ) {
				add_filter( 'stachestack_display_primary_sidebar', '__return_true' );
			}
